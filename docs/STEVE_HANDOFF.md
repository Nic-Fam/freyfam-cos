...
Unlike the other specialists (metered Anthropic API via `src/claude.js`), **you run
on a Claude Code subscription** — your runner shells out to headless `claude -p`
(`src/specialists/dev-claude-code.js`, gated by `COS_DEV_BACKEND=claude-code` in the
`DEV` block of `src/config.js`). That gives you real file/bash/build tools.

- You operate in **`COS_DEV_CWD`** (your workspace), never on the family's behalf.
- **Critical gotcha:** `ANTHROPIC_API_KEY` in your env shadows the subscription and
  silently bills the API. Keep it unset. (`subscriptionEnv()` also scrubs it from the
  child.) Full detail + launchd setup: `deploy/setup/steve-macbook.md`.
- The `delegate` contract `{agent, task} -> text` is unchanged regardless of backend.
...
src/specialists/runner.js     transport-agnostic specialist execution core
src/specialists/dev-claude-code.js   YOUR backend (Claude Code subscription)
...
4. **Specialists return text only.** A specialist (including you) has no outbound
   ...
   on Lloyd. Your file/bash tools act in `COS_DEV_CWD`, never as the family.
...
- `node _smoke.mjs` is a zero-network smoke (guards + confirm parser + memory
  round-trip). Run it anytime; no creds needed.
...
- Node caches imported modules — **new code only loads on restart.** After pulls or
  `.env` changes, follow the safe-restart in `deploy/setup/restart.md`.
...
- Runs under **launchd** (`deploy/com.freyfam.cos.plist`, `com.freyfam.steve` for you),
  `caffeinate` baked in, KeepAlive + RunAtLoad.
