...
- `request_specialist` → Lloyd `delegate`s to the named specialist (e.g. **Steve for a
  dev push**). Routine help.
- `request_heavy_lift` → a **confirmation-gated ask to Nic** to run a scoped task on his
  Claude Code subscription (reuses workstream Q's subscription backend — `DEV` /
  `dev-claude-code.js`). Serious dev/build work, flat-rate, human-initiated.
...
- [ ] **2. Request seam.** COO request tools → Lloyd router; `request_specialist` (→Steve),
      `request_heavy_lift` (→gated ask to Nic, Q backend), `request_action` (→`confirm.js`).
...
- Parallel-safe: mostly additive (new roster loader, new persona, new `cost-ledger.js`,
  ...
  E (memory), F (specialist tools), J (cost watchdog), O (personas), Q (subscription dev).
